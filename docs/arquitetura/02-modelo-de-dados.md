...
`insumos_master.tipo` teve **129 linhas `comercial` e 1 `producao_interna`** na leitura de 14/08/2026, e essas
duas contagens somam 130 contra as **131** linhas registradas na mesma leitura. Falta uma linha, `tipo` é
`NOT NULL`, e nenhum documento resolve isso — só o banco (`A24`).

**A view não depende mais dessa resposta.** Ela descia por `tipo = 'producao_interna'` enquanto decidia folha
por *ter lista própria*, e um insumo com lista e outro rótulo caía no vão: não era descido nem contado como
folha, sumia da conta, e o custo do prato saía menor e sem aviso. Desde
[`20260817116000_custo_por_fato_e_nao_por_rotulo.sql`](../../supabase/migrations/20260817116000_custo_por_fato_e_nao_por_rotulo.sql)
os dois lados usam **o mesmo predicado**: tem lista própria em `producao_ingredientes`, ou não tem. O valor de
`tipo` não entra no cálculo.

A pergunta continua valendo para o **cadastro**, e virou consulta: `vw_custo_insumo_suspeito` enumera os valores
de `tipo` que existem, marca o que estiver fora dos dois, e conta onde rótulo e fato discordam. A aba de pratos
mostra o resultado quando há o que mostrar.
...
### 5.5 Onde está o SQL, e por que ele não está copiado aqui

**A view vive em
[`20260817116000_custo_por_fato_e_nao_por_rotulo.sql`](../../supabase/migrations/20260817116000_custo_por_fato_e_nao_por_rotulo.sql),
e é a única versão dela.** Esta seção trazia as ~180 linhas do corpo copiadas, e a cópia envelheceu duas vezes
em silêncio: ficou para trás na correção de ciclo (`A25`, migration `113000`) e de novo na correção de rótulo
(`A24`, migration `116000`). Quem lesse esta seção para conferir o cálculo conferiria contra uma view que não
existe mais, e concluiria que o **código** estava errado.

Copiar SQL para dentro de documento é a mesma classe de erro que este documento denuncia em outros lugares: uma
segunda fonte da verdade, sem nada que force as duas a concordarem. A folha canônica tem precedência sobre o
código para **decisões**; para o **corpo de uma view**, a migration é a fonte, e o documento aponta.

O que fica aqui é o que a migration não diz de forma tão compacta — a forma do algoritmo e o porquê de cada
trava:

| Etapa | O que faz | Por quê |
|---|---|---|
| `arvore` (âncora) | Uma linha por ingrediente direto do prato, com `quantidade / coalesce(rn_override, rn)` | `rn_override` da linha vence o `rn` do insumo (P2, fato verificado) |
| `arvore` (recursão) | Desce em quem **tem lista própria** em `producao_ingredientes`, dividindo pelo `rendimento` do lote do pai | Descer por `tipo = 'producao_interna'` fazia insumo com lista e outro rótulo sumir da conta inteira (`A24`) |
| Trava de ciclo | Ramo cujo insumo já apareceu **no mesmo caminho** é emitido marcado, e não descartado | Descartado, o custo saía menor e plausível (`A25`). `visitados` acumula por caminho, então o mesmo insumo em dois ramos distintos é somado duas vezes, corretamente |
| Trava de profundidade | Teto de 5 níveis, e o corte fica marcado em `insumos_truncados` | A ficha de hoje tem 2 níveis; o teto é folga, e cortar em silêncio seria custo menor sem aviso |
| `folha` | Nó sem lista própria, ou marcado por ciclo, ou no teto | **O mesmo predicado da recursão**, e é essa simetria que fecha o vão do `A24` |
| `datas` | As datas em que o preço de algum insumo da árvore mudou, mais hoje | O custo é função em degraus, e muda exatamente nessas datas (seção 5.4) |
| `preco` | Média das linhas da última data com preço, com `preco_unitario_fixo` de reserva | Duas notas na mesma data são normais; desempatar por `uuid` fazia o custo depender de qual identificador saiu maior (`A26`) |
| `agregado` | Conta os buracos: sem preço, sem rendimento, truncados, em ciclo | `sum()` ignora `NULL`, então a soma sozinha nunca revela buraco nenhum |
| `resolvido` | **Reintroduz `NULL`** se qualquer contagem de buraco for diferente de zero, com `motivo_incompleto` em português | Custo zero num painel de margem não parece erro: parece prato muito lucrativo |

As premissas não verificadas (`N46`) estão na seção 5.2, e viajam com o número até a tela em `nota_premissa`.
